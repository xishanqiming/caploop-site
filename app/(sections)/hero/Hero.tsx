      ...
      <div className="container grid md:grid-cols-2 gap-10 items-center">
        <div>
          ...
          <h1 className="mt-5 text-4xl md:text-5xl font-semibold leading-tight">
            {onepager.title.split("SME")[0]}
            <span className="highlight">SME</span>
            {onepager.title.split("SME")[1] || ""}
          </h1>
          ...
          <div className="mt-8 flex gap-3">
            <Button asChild href="#structure">
              了解流程
            </Button>
            <Button variant="ghost" asChild href="#contact">
              预约/申请
            </Button>
            <DownloadDialog />
          </div>
              ...
              {[
                ...
                <li key={i} className="flex items-center gap-2">
                  <span className="inline-flex size-5 rounded-full bg-white/70 border border-white justify-center items-center">
                    <Check className="size-5 text-[hsl(var(--accent-600))]" />
                  </span>
                  {x}
                </li>
              ))}
